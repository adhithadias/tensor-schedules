...
if known_plot:
    df.plot(y = ['sparseShed', 'default'], ax = ax, kind='bar', yerr=yerr, error_kw=dict(ecolor='k'), log=True, rot = 45, ylim = ax1_ylim, ylabel = "Execution Time (ms)", legend = False)
else:
    df.plot(y = ['sparseShed', 'default'], ax = ax, kind='bar', yerr=yerr, error_kw=dict(ecolor='k'), log=True, rot = 45, ylabel = "Execution Time (ms)")
...
if known_plot:
    df.plot(x = 'Tensor', y = ['speedup'], ax = ax2, linestyle='-', marker='o', color = 'black', secondary_y=True, rot = 45, ylim = ax2_ylim, ylabel = "Speedup", xlabel = "Matrix/Tensor", legend = False)
else:
    df.plot(x = 'Tensor', y = ['speedup'], ax = ax2, linestyle='-', marker='o', color = 'black', secondary_y=True, rot = 45, ylabel = "Speedup", xlabel = "Matrix/Tensor")
# plt.ylabel("speedup")
