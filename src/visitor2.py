...
class PrintConfigVisitor(Visitor):
    def __init__(self, tensor_accesses: dict, out_file=(sys.stdout)):
        self.tabs = 0
    ...
    def visit(self, config):
        ...
        print('\t'*self.tabs, '|',
              output, '=',
              tensor_contraction,
              ', fsd: ' + str(config.fused) + ", pol: " +
              str(config.prod_on_left),
              '| lp_ord:', config.input_idx_order, '|', config.time_complexity, ',', config.memory_complexity, file=self.out_file)
        self.tabs += 1
        if (config.prod != None):
